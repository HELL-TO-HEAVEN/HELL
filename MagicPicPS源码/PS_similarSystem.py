...
注意的地方：
1、图片选择错误的话会自动异常退出
2、程序报出log4cplus ：ERROR xxx等等，可以不用理会，不影响运行
"""
...
# 图片区域：
        ...
        self.picArea.setStyleSheet("background-color:rgb(43,43,43);background-image:url(pic/bg.png);")    ##@@@@这里加了：
...
"font-family:微软雅黑;background-color:transparent;background-image:url(images/minus1.png);")
        ...
        self.rotate_Button.setGeometry(QtCore.QRect(60, 95, 141, 51))           # QRect（左上角，宽，高）
...
#——————————————组件翻译函数：——————————————————————————————
    def retranslateUi(self, MainWindow): # 这个函数好像是专门给组件命名的。
        ...
        self.toolBox.setItemText(self.toolBox.indexOf(self.page1), _translate("MainWindow", "基本工具"))
        ...
        self.footerLabel1.setText(_translate("MainWindow", "您已经在MagicPicPS花费了"))
...
## 拼接：
    def together(self):
        address=QFileDialog.getOpenFileNames(self,"请选择要拼接的图片（2张）(*^__^*)","C:/")
        if address[0]:
...
# 生成字符画：
    ...
    def createCodePic(self):
        ...
        for i in range(height):
            ...
            txt = txt + "\n"
        print("2")
        txtAddress = QFileDialog.getSaveFileName(self, "选择一个路径把你的字符画收好！（以txt格式存储）", "C:/Magpic pictures",
    ...
    def compare(self):
        address = QFileDialog.getOpenFileNames(self, "请选择要比较的图片(*^__^*)（2张）", "C:/")
        if address[0]:
            num = len(address[0])  # 因为这个address的第一项才存放的地址。
            print("有", num, "个文件被选出来了：")
            pic1=address[0][0]
